...
int main() {
  ...
  // op type
  const std::string op_type = "FillV2";
  // input - dims
  ...
  // input - dims
  auto input = new npuTensor<int64_t>(ACL_INT64, input_dims.size(), input_dims.data(), ACL_FORMAT_NCHW, input_data.data(), memType::HOST);
